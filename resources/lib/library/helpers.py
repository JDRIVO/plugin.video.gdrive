...
def updateLibrary(filePath, metadata):
	...
	dbPath = getVideoDB()
	db = Database(dbPath)
	fileID = db.select("files", "idFile", {"idPath": f'(SELECT idPath FROM path WHERE strPath="{dirPath + os.sep}")', "strFilename": filename})
	...
	if db.selectAll("streamdetails", condition):
